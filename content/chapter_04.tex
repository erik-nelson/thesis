...
As it explores, a robot is constantly evaluating reward in new areas of the
...
Computing $\text{I}_{\text{CS}}(\mbf{m}\, ;\, C(\mbf{m}))$ requires substituting
...
distributions on the bottom-most row and right-most column. \comment{Explain
coefficients better}.
...
%During exploration, a robot will continuously plan actions to new locations in its map
%and evaluate an information-based reward function to select its next movement. So far,
%the PRI strategy from Chapter~\ref{chapter3} can be used to compress the map, increasing the
%efficiency of evaluating CSQMI reward. In addition, the IB
%method from Section~\ref{sec:ib_spec} can be used to choose an
%amount of compression that causes CSQMI reward to suffer a minimal
